"""Breadcrumb module for entry templatetags"""
import re
...
def year_crumb(creation_date):
    ...
    return Crumb(year, reverse('entry_entry_archive_year',
                               args=[year]))
...
def month_crumb(creation_date):
    ...
    return Crumb(month_text, reverse('entry_entry_archive_month',
                                     args=[year, month]))
...
def day_crumb(creation_date):
    ...
    return Crumb(day, reverse('entry_entry_archive_day',
                              args=[year, month, day]))
...
MODEL_BREADCRUMBS = {'Tag': lambda x: [Crumb(_('Tags'),
                     ...
                     'Author': lambda x: [Crumb(_('Authors'),
                                              reverse('entry_author_list')),
                                        Crumb(x.username)],
                     'Category': lambda x: [Crumb(
                         _('Categories'), reverse('entry_category_list'))] + \
...
def retrieve_breadcrumbs(path, model_instance, root_name=''):
    ...
    based of the model's url handled by entry"""
    breadcrumbs = []

    if root_name:
        breadcrumbs.append(Crumb(root_name, entry_ROOT_URL()))
    ...
    url_components = [comp for comp in
                      path.replace(entry_ROOT_URL(), '').split('/') if comp]
